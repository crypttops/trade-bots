            ...
            <div class="bots-config-table">
              <Table border :columns="botHeaders" :data="configuredBots">
                <template slot-scope="{ row, index }" slot="action">
                  <Button type="info" size="small" style="margin-right: 5px" @click="editBot(row.id, row.botType,row, row)">Edit</Button>
                  <Button type="warning" size="small" @click="stopBot(row.id, row.botType, index)" v-if="row.active">Stop Bot</Button>
                  <Button type="success" size="small" @click="startBot(row.id, row.botType, index)" v-if="!row.active">Start Bot</Button>
                  <Button type="error" size="small" @click="deleteBot(row.id, row.botType)">Delete Bot</Button>
              </template>
              </Table>
            </div>
          </div>
          ...
          <div class="side-panel">
            <Tabs :animated="false">
              <TabPane label="Strategy">
                <div style="padding: .5em 1em;border-bottom:1px solid;cursor:pointer" v-for="strategy in strategyList" :value="strategy" :key="strategy">{{ strategy }}</div>
              </TabPane>
              <TabPane label="Balance">
                <Card class="bg-color">
                  ...
                  <List size="small" border>
                      <ListItem item-layout="horizontal" v-for="item in exchangesBalances.spot" :key="item.asset" style="list-style-type:none; color:#fff;">
      ...
      <div class="flex-col-2">
        <div>
          ...
          <Select v-model="dcaConfigs.exchange_id">
            <Option v-for="exchange in exchangeList" :value="exchange.id" :key="exchange.name">{{ exchange.name }}</Option>
          </Select>
        </div>
         <div>
          ...
          <div class="flex-col">
            <div class="flex-col-2">
               <i-switch v-model="dcaConfigs.trailing_stop_enabled" @on-change="enableTrailingInput" />
            </div>
         ...
         <div>
          ...
          <Select v-model="dcaConfigs.strategy">
            <Option v-for="strategy in strategyList" :value="strategy" :key="strategy">{{ strategy }}</Option>
          </Select>
        </div>
        <div>
...
export default {
  ...
  methods: {
    getAllBots(userId) {
        ...
        let dcas = []
        let gridbots = []
        bitsgapInstance.get(`${bitsgap.dcaBots}${userId}`)
        .then(
        response => {
          const dcaBots =response.data.result;

          bitsgapInstance.get(`${bitsgap.gridBots}${userId}`)
          .then((grid) => {
            ...
            for (var i = 0; i < allBots.length; i++) {
              ...
              bot.is_running == true ? bot.running = 'Running' : bot.running = 'Inactive';
              bot.is_running == true ? bot.active = true : bot.active = false;
              bot.exchange = this.exchangeList.find(e => e.id == bot.exchange_id).type;
            }
    ...
    startGridBot(botId){
      ...
      bitsgapInstance.post(bitsgap.gridBotStart, payload)
      .then(response => {
    ...
    createDCABot() {
      bitsgapInstance.post(bitsgap.dcaBot, this.dcaConfigs)
      .then(response => {
    ...
    startDCABot(botId) {
      ...
      bitsgapInstance.post(bitsgap.dcaBotStart, payload)
      .then(response => {
        this.$Message.success({
    ...
    stopDCABot(botId) {
      ...
      bitsgapInstance.post(bitsgap.dcaBotStop, payload)
      .then(response => {
        this.$Message.success({
    ...
    stopGridBot(botId) {
      ...
      bitsgapInstance.post(bitsgap.gridBotStop, payload)
      .then(response => {
        this.$Message.success({
    ...
    deleteDCABot(botId) {
      ...
      bitsgapInstance.delete(`${bitsgap.dcaBot}?userId=${this.userId}&bot_id=${botId}`)
      .then(response => {
        this.$Message.success({
    ...
    deleteGridBot(botId) {
      ...
      bitsgapInstance.delete(`${bitsgap.createGridBot}?userId=${this.userId}&bot_id=${botId}`)
      .then(response => {
    ...
    editDCABot(dcaConfigs) {
      ...
      bitsgapInstance.put(bitsgap.dcaBot, updated)
      .then(response => {
          this.$Message.success({
          content: this.dcaConfigs.bot_name + " " + "DCA Bot updated successfully",
          duration: 5
          });
          this.getAllBots(this.userId)
      }).catch(error => {
    ...
    editGridBot(gridConfigs) {
      ...
      bitsgapInstance.put(bitsgap.createGridBot, updatedata)
      .then(response => {
          this.getAllBots(this.userId)
          this.$Message.success({
          content: this.gridConfigs.botName + " " + "GRID Bot updated successfully",
          duration: 5
          });
      }).catch(error => {
    ...
    createGridBotEndpoint() {     
      bitsgapInstance.post(bitsgap.createGridBot, this.gridConfigs)
      .then(response=>{
    ...
    loadStrategies(){
      bitsgapInstance.get(bitsgap.strategies)
      .then(response => {
    ...
    loadExchanges(){
      bitsgapInstance.get(bitsgap.exchanges+ "?userId=" + this.userId)
      .then(response => {
    ...
    loadExchangesBalances(){
        ...
        bitsgapInstance.post(bitsgap.exchangeBalances, payload)
        .then(response => {
