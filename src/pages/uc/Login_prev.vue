...
export default {
  ...
  methods: {
    ...
    initGtCaptcha() {
      ...
      .catch((error) => {
        console.log('an error occured starting the captcha', error)
      });
    },
    handler(captchaObj) {
      console.log('%chandling some things', "background-color:red;color:#fff")
      captchaObj.onReady(() => {
          $("#wait").hide();
        }).onSuccess(() => {
          let result = (this._captchaResult = captchaObj.getValidate());
          if (!result) {
            this.$Message.error("Please complete verification");
          } else {
            this.handleSubmit("formInline");
          }
        });
      $(".login_btn").click(() => {
        console.log('probably clicked the login button')
